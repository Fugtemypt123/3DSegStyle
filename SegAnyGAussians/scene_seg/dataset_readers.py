...
from ..utils_seg.graphics_utils import getWorld2View2, focal2fov, fov2focal
import numpy as np
import torch
import json
from pathlib import Path
from plyfile import PlyData, PlyElement
from ..utils_seg.sh_utils import SH2RGB
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None, mask_scale_folder = None, sample_rate = 1.0, allow_principle_point_shift = False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        # allocated_memory = torch.cuda.memory_allocated(0) / (1024**2)  # Convert bytes to MB
        # # gpu_name = torch.cuda.get_device_name(gpu_id)
        # print(f"Allocated memory before: {allocated_memory:.2f} MB")
        # total_memory = torch.cuda.get_device_properties(gpu_id).total_memory / (1024**2)  # Convert bytes to MB
        ...
        features = torch.load(os.path.join(features_folder, image_name.split('.')[0] + ".pt"), map_location=torch.device('cpu')) if features_folder is not None else None
        masks = torch.load(os.path.join(masks_folder, image_name.split('.')[0] + ".pt"), map_location=torch.device('cpu')) if masks_folder is not None else None
        mask_scales = torch.load(os.path.join(mask_scale_folder, image_name.split('.')[0] + ".pt"), map_location=torch.device('cpu')) if mask_scale_folder is not None else None
