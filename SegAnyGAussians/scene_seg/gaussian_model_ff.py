...
import numpy as np
from ..utils_seg.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
from ..utils_seg.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from ..utils_seg.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from ..utils_seg.graphics_utils import BasicPointCloud
from ..utils_seg.general_utils import strip_symmetric, build_scaling_rotation
...
class FeatureGaussianModel:
    ...
    def get_smoothed_point_features(self, K = 16, dropout = 0.5):
        ...
        # normed_features = normed_features.detach().cpu()
        # print("Normed_features Device: ", normed_features.device)

        if dropout > 0 and dropout < 1:
            select_point = torch.randperm(K)[ : int(K*dropout)]

            select_idx = self.feature_smooth_map["m"][:, select_point]
            ret = normed_features[select_idx, :].mean(dim = 1) # .to("cpu")
        else:
            ret = normed_features[self.feature_smooth_map["m"], :].mean(dim = 1) # .to("cpu")
    ...
    def save_ply(self, path, smooth_weights = None, smooth_type = None, smooth_K = None):
        ...
        self._point_features = self._point_features.detach().cpu()
        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        # print("Point features shape:", self._point_features.shape)
        # print("xyz shape:", xyz.shape)
        # print("Normals shape:", normals.shape)
        # print("Feature smooth:", self.feature_smooth_map)
